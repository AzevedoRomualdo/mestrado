  ...
  <section class="content" data-app>
    ...
    <v-card style="margin-left: 30vh; margin-top: 15vh; margin-right: 30vh">
      <v-layout row wrap style="margin-left:46vh; ">
        <span style="color:black; font-size:46px; ">Editar cadastro de Usuário</span>
      </v-layout>
  ...
  methods: {
    ...
    alterar: function() {
      if (this.$route.params.id) {
        ...
        if (
          ...
          axios
            .put("editusuario/" + this.usuario.id)
            .then(response => {
              this.msgServico = "Usuario alterado com sucesso";
    ...
    carregar() {
      if (this.$route.params.id) {
        this.id = this.$route.params.id;
        axios
          .get("usuarioedit/" + this.id)
          .then(response => {
            console.log(response.data);
            this.usuario.id = response.data[0].id;
            this.usuario.nome = response.data[0].nome;
            this.usuario.tipo = response.data[0].tipo;
            this.usuario.matricula = response.data[0].matricula;
            this.usuario.universidade = response.data[0].universidade;
            this.usuario.senha = response.data[0].senha;
