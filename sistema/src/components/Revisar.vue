      ...
      <v-tooltip bottom>
        <template v-slot:activator="{ on }">
          <v-flex v-on="on" xs3 sm3 style="margin-top:4vh">
            <h1>Atividades para Revisão</h1>
          </v-flex>
        </template>
        <span>
          Nesta tela você encontra a lista de atividades a serem revisadas NESTA RODADA e
          respectivamente suas turmas e períodos em que estão associados.
        </span>
      </v-tooltip>
    </v-layout>
    <!--  -->
...
export default {
  ...
  methods: {
    revisarAtividade: function(item) {
      ...
      let url = "/revisao/" + this.usuario[0].id + "/" + item.id_atv;
